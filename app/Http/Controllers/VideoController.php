...
class VideoController extends Controller
{
    ...
    {
        $input = $request->all();
        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->VideoRepository->model());
        try {
            $video = $this->VideoRepository->create($input);
            $video->customFieldsValues()->createMany(getCustomFieldsValues($customFields, $request));
            if (isset($input['video']) && $input['video']) {
                $cacheUpload = $this->uploadRepository->getByUuid($input['video']);
                $mediaItem = $cacheUpload->getMedia('video')->first();
                $mediaItem->copy($video, 'video');
            }
    ...
    //    return $eProvider;
        ...
        return view('videos.edit')->with('gallery', $gallery)->with('eProvider', $eProvider)->with("customFields", isset($html) ? $html : false)->with("eProvider", $eProvider);
    }
