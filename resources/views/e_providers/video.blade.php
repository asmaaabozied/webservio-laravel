...
<div class="d-flex flex-column col-sm-12 col-md-6">
    ...
    <div class="form-group align-items-start d-flex flex-column flex-md-row">
        {!! Form::label('video', trans("lang.e_provider_video"), ['class' => 'col-md-3 control-label text-md-right mx-1']) !!}
        <div class="col-md-9">
            <div style="width: 100%" class="dropzone image" id="image" data-field="image">
                <input type="hidden" name="video">
            </div>
            <a href="#loadMediaModal" data-dropzone="image" data-toggle="modal" data-target="#mediaModal" class="btn btn-outline-{{setting('theme_color','primary')}} btn-sm float-right mt-1">{{ trans('lang.e_provider_video_select')}}</a>
            <div class="form-text text-muted w-50">
                {{ trans("lang.e_provider_video_help") }}
            </div>
        </div>
    </div>
    @prepend('scripts') 
...
<div class="form-group col-12 d-flex flex-column flex-md-row justify-content-md-end justify-content-sm-center border-top pt-4">
    <button type="submit" class="btn bg-{{setting('theme_color')}} mx-md-3 my-lg-0 my-xl-0 my-md-0 my-2">
        <i class="fas fa-save"></i> {{trans('lang.save')}} {{trans('lang.e_provider_video')}}</button>
    <a href="{!! route('eProviders.index') !!}" class="btn btn-default"><i class="fas fa-undo"></i> {{trans('lang.cancel')}}</a>
</div>
