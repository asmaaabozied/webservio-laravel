    ...
    <div class="card shadow-sm">
        <div class="card-header">
            <ul class="nav nav-tabs d-flex flex-row align-items-start card-header-tabs">
                ...
                @can('galleries.edit')
                <li class="nav-item">
                    <a class="nav-link active" href="{!! url()->current() !!}"><i class="fas fa-edit mr-2"></i>{{ trans('lang.gallery') }}</a>
                </li>
                @endcan
                {{-- @can('videos.edit')
                    <li class="nav-item">
        ...
        <div class="card-body">
            <div class="row">
                <div class="d-block">
                    <button class="btn btn-primary ml-5 " id="create_award_btn">Add Gallery</button>
                </div>
                <div class="create_modal d-none">
                    {!! Form::open(['route' => 'galleries.store']) !!}
                    <div class="row">
                        @include('galleries.provider_fields')
                    </div>
                    {!! Form::close() !!}
                    <div class="clearfix"></div>
                </div>
            ...
            @if ($galleries)
            <div class="p-4 m-2">
                <table id="NoGallery" class="display table" style="width:100%">
                    <thead>
                        <tr>
                            <th>Image</th>
                            <!-- <th>Description</th> -->
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <img class=" rounded " style="height:50px" src="{{$gallery->media->first()->url}}" alt="{{$gallery->media->first()->name}}">
                            </td>
                            ...
                            <td>
                                ...
                                <!-- <div class='btn-group btn-group-sm'>
                                    @can('galleries.edit')
                                    <a data-toggle="tooltip" data-placement="left" title="{{trans('lang.gallery_edit')}}" href="{{ route('galleries.edit', $gallery->id) }}" class='btn btn-link'>
                                        <i class="fas fa-edit"></i> </a> @endcan

                                    @can('galleries.destroy') {!! Form::open(['route' => ['galleries.destroy', $gallery->id], 'method' => 'delete']) !!} {!! Form::button('<i class="fas fa-trash"></i>', [ 'type' => 'submit', 'class' => 'btn btn-link text-danger', 'onclick' => "return confirm('Are you sure?')" ]) !!} {!! Form::close() !!} @endcan
                                </div> -->
                                <div class='btn-group btn-group-sm'>

                                <a  href="{!! route('galleries.edit',$gallery->id) !!}"><i class="fa fa-edit mr-2"></i></a>
