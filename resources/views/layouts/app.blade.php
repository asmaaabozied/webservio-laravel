...
<!-- Content Wrapper. Contains page content -->
    ...
    <footer class="main-footer border-0 shadow-sm">
        <div class="float-sm-right d-none d-sm-block">
            <b>{{trans('lang.Version')}}</b> {{implode('.',str_split(substr(config('installer.currentVersion','v100'),1,3)))}}
        </div>
        <strong>{{trans('lang.Copyright')}} © {{date('Y')}} <a href="{{url('/')}}">{{setting('app_name')}}</a>{{trans('lang.All_rights_reserved')}}.</strong>
    </footer>
